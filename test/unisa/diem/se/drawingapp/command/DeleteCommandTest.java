/**
 * SE_DrawingApplication
 * 
 * Group members:
 *  ⋅ Amato Emilio
 *  ⋅ Apicella Salvatore
 *  ⋅ Bove Antonio
 *  ⋅ Cerasuolo Cristian
 */

package unisa.diem.se.drawingapp.command;

import javafx.scene.layout.Pane;
import org.junit.Test;
import static org.junit.Assert.*;
import org.junit.Before;
import unisa.diem.se.drawingapp.controller.DrawingSurfaceManager;
import unisa.diem.se.drawingapp.shape.*;
import unisa.diem.se.drawingapp.utility.UtilityTest;

public class DeleteCommandTest {

    private  DeleteCommand command;
    private  Pane drawingPane;
    private  RectangleShape addedShape;
    
    @Before
    public void setUp(){
        this.drawingPane = UtilityTest.createAndSetPane();
        this.addedShape = new RectangleShape(UtilityTest.POS, UtilityTest.POS, UtilityTest.TEST_WIDTH_SHAPE, UtilityTest.TEST_HEIGHT_SHAPE);
        this.command = new DeleteCommand(this.addedShape);
    }
    

    /**
     * Test of execute method, of class DeleteCommand.
     */
    @Test
    public void testExecute() {
        System.out.println("execute");
        
        //TEST: Add on pane the shape and delete it with execute method from DeleteCommand class, 
        //checking if the size of pane's list of shape is 1 smaller than the original one 
        
        this.addedShape.draw();
        int prevSize = this.drawingPane.getChildren().size();
        
        this.command.execute();
        
        assertFalse(DrawingSurfaceManager.getInstance().getDrawingPane().getChildren().contains(this.addedShape.getShape()));
        assertEquals(prevSize - 1, this.drawingPane.getChildren().size());     
    }

    /**
     * Test of undo method, of class DeleteCommand.
     */
    @Test
    public void testUndo() {
        System.out.println("undo");
        
        //TEST: add on pane the shape and delete it with 'execute' method from DeleteCommand class, then execute the 'undo' method and check if 
        //the size of Pane's list of shape is equal to the original before the execution of command 
        this.addedShape.draw();
        int originalSize = this.drawingPane.getChildren().size();
        
        this.command.execute();
        this.command.undo();
        
        assertTrue(DrawingSurfaceManager.getInstance().getDrawingPane().getChildren().contains(this.addedShape.getShape()));
        assertEquals(originalSize,this.drawingPane.getChildren().size());
    }

}